...
Syntax::Kamelon - A versatile and fully programmable textual content parser that is extremely well suited 
for syntax highlighting and code folding 
...
highlight definitions. Also I have extended Kates' xml definitions to make it usefull outside
the editor environement.
...
Here you specify what Kamelon should return for every attribute tag. Every key in the hash corresponds
to an available attribute tag.

If you use the B<Highlight> method it can be anything you want. I have an application 
running that holds an Wx::TextStyle object to every attribute tag. 
...
=item B<GetHighlighter>(I<$syntax>);

Returns the B<Highlighter> object from the pool of loaded B<Highlighter> objects. If it is
...
Parses I<$text> and returns a paired list of text fragments and the format information from B<FormatTable>.

=item B<HighlightAndFormat>(I<$text>);

Parses I<$text> and returns a formatted text. Usefull for creating HTML output.
...
Called by B<Highlight>. It is given one line (including the newline) as a parameter.
